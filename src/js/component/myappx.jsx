...
const MyAppX = () => {
    ...
    const [serverUrl, setServerUrl] = useState("https://assets.breatheco.de/apis/fake/todos/user/swedishchef27"); // Need to put URL in useState to be able to pass it to useEffect
    ...
    useEffect( () => {
        ...
        async function fetchData(serverUrl){
            ...
            console.log(resp)
            // Create a New user if response from getData is not ok
            if (!resp.ok){
                console.log("Could not get data from username")
                await createUser(serverUrl);
            }
            // Otherwise convert data from API to json & change State to the fetched todo list 
            else {
			...
			<div className="d-flex justify-content-center">
				<ul className="list-group w-50">
                    ...
                    {todoArray.map( (item, index) => {
                        console.log("Converting to-do list to HTML:")
                        ...
                        if (item.label === "sample task"){
                            ...
                            return <li className="list-group-item list-style d-inline-flex justify-content-between" key={index}>{item.label}
                                        ...
                                        <div className="d-inline-flex justify-content-between"  style={{display:"inline", width:"15%"}}>
